...
const viewEmp = async () => {
	...
	FROM
		employeedirectory_db.employees AS e
			INNER JOIN
		roles ON (e.role_id = roles.id)
			INNER JOIN
		departments ON (roles.dept_id = departments.id)
			LEFT JOIN
		employeedirectory_db.employees m ON e.manager_id = m.id;`
    );
...
const viewAllRoles = async () => {
    const roleTable = await query(
        ...
        r.title AS 'Title'
    FROM 
        employeedirectory_db.roles AS r 
            INNER JOIN 
            ...
            },
            {
                ...
                validate: function (value) {
                    if (!value || !value.match(/^\d+/)) {
                        console.log('Enter a valid salary for the role.');
                        return false;
                    }
                    return true;
                },
